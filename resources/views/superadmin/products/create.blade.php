    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Add New Product</h2>

                <form method="POST" action="{{ route('superadmin.products.store') }}" enctype="multipart/form-data"
                    class="space-y-5">
                    @csrf
                    ...
                    @if(count($categories) > 0 && count($models) > 0)
                        <div x-data="{
                                    ...
                                    selectedCategoryId: '{{ old('snipeit_category_id') }}',
                                    categories: {{ Js::from($categories) }},

                                    // Model state
                                    modelOpen: false,
                                    modelSearch: '',
                                    selectedModel: '{{ old('model_name') }}',
                                    selectedModelId: '{{ old('snipeit_model_id') }}',
                    ...
                    @elseif(count($categories) > 0)
                        ...
                        <div x-data="{ 
                                    ...
                                    selectedId: '{{ old('snipeit_category_id') }}',
                                    categories: {{ Js::from($categories) }},
                                    get filtered() {
                    ...
                    <div>
                        ...
                        <textarea name="specs" rows="3" required
                            class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-indigo-500 focus:border-indigo-500">{{ old('specs') }}</textarea>
                            ...
                            @foreach($requestTypes as $type)
                                <label class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer transition">
                                    <input type="checkbox" 
                                        name="request_types[]" 
                                        value="{{ $type->id }}"
                                        {{ in_array($type->id, old('request_types', [])) ? 'checked' : '' }}
