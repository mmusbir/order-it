<x-sidebar-layout>
    <x-slot name="header">
        <div class="flex items-center text-sm text-gray-500">
            ...
            <span class="text-indigo-600 font-semibold">Asset Models</span>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Asset Model Management</h1>
                    <p class="text-gray-500 mt-1">Manage asset models and sync with Snipe-IT.</p>
                </div>
                <div class="flex gap-3 mt-4 md:mt-0">
                    <form action="{{ route('superadmin.settings.asset-models.sync') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($assetModels as $model)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-2 py-1 text-xs font-medium rounded-full {{ $model->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <button type="button"
                                            onclick="openEditModal({{ $model->id }}, '{{ addslashes($model->name) }}', {{ $model->category_id ?? 'null' }}, {{ $model->is_active ? 'true' : 'false' }})"
                                            class="text-indigo-600 hover:text-indigo-900 text-sm font-medium">Edit</button>
                                        <form action="{{ route('superadmin.settings.asset-models.destroy', $model) }}"
                                            method="POST" class="inline" onsubmit="return confirm('Hapus model ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                    No asset models found. Add a new model or sync from Snipe-IT.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="addModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Add Asset Model</h3>
            <form action="{{ route('superadmin.settings.asset-models.store') }}" method="POST">
                @csrf
                <div class="mb-4">
    ...
    <div id="editModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Edit Asset Model</h3>
    ...
    <script>
        function openEditModal(id, name, categoryId, isActive) {
            document.getElementById('editForm').action = '/superadmin/settings/asset-models/' + id;
