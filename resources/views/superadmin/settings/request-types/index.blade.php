<x-sidebar-layout>
    <x-slot name="header">
        <div class="flex items-center text-sm text-gray-500">
            ...
            <span class="text-indigo-600 font-semibold">Request Types</span>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Request Type Management</h1>
                    <p class="text-gray-500 mt-1">Kelola jenis-jenis request yang tersedia di form.</p>
                </div>
                        ...
                        @forelse($requestTypes as $type)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                    <code
                                        class="bg-gray-100 dark:bg-gray-600 px-2 py-1 rounded text-xs">{{ $type->slug }}</code>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                    {{ $type->description ?? '-' }}
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if($type->allow_quantity)
                                        <span
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-2 py-1 text-xs font-medium rounded-full {{ $type->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <button type="button"
                                            onclick="openEditModal({{ $type->id }}, '{{ addslashes($type->name) }}', '{{ $type->slug }}', '{{ addslashes($type->description ?? '') }}', {{ $type->allow_quantity ? 'true' : 'false' }}, {{ $type->is_active ? 'true' : 'false' }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                    Belum ada request type. Tambahkan request type baru.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="editModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Edit Request Type</h3>
            <form id="editForm" method="POST">
                ...
                <div class="mb-4">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="allow_quantity" id="editAllowQuantity" value="1"
                            class="rounded text-blue-600">
                        <span class="text-sm text-gray-700 dark:text-gray-300">Enable Quantity Input</span>
                    </label>
                    <p class="text-xs text-gray-500 ml-6 mt-1">Jika diaktifkan, user dapat mengatur quantity per item di
                        form request</p>
                </div>
                <div class="mb-4">
                    <label class="flex items-center gap-2">
    ...
    <script>
        function openEditModal(id, name, slug, description, allowQuantity, isActive) {
            document.getElementById('editForm').action = '/superadmin/settings/request-types/' + id;
            document.getElementById('editName').value = name;
            document.getElementById('editSlug').value = slug;
            document.getElementById('editDescription').value = description;
            document.getElementById('editAllowQuantity').checked = allowQuantity;
